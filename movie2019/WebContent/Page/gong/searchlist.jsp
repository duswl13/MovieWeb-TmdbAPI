...
<c:otherwise>
		<c:forEach var="row" items="${list}" varStatus="status">
			...
			<tr>
			<td colspan="2"><div id="divresult${status.count }" style="display: none; text-align: center;">${row.NOTICE_CONTENT }</div></td>
			</tr>
	...
	<tr>
	<td colspan="2">
	<c:if test="${current_page>1 }"><a href="#" onclick="gongSearch('1')">[처음]</a></c:if>
	<c:if test="${current_block>1 }">
	<a href="#2" onclick="gongSearch('${prev_page}')">[이전]</a>
	</c:if>
	...
	<!-- 현재페이지가 아니면 하이퍼링크 표시 -->
	<c:if test="${page!=current_page }">
	<a href="#1" onclick="gongSearch('${page}')">[${page}]</a>
	</c:if>
	</c:forEach>
	<c:if test="${current_block<total_block }">
	<a href="#4" onclick="gongSearch('${next_page}')">[다음]</a>
	</c:if>
	<c:if test="${current_page<total_page }">
	<a href="#5" onclick="gongSearch('${total_page}')">[끝]</a>
	</c:if>
	</td>
	</tr>
	</table>
