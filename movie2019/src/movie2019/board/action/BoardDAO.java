...
public class BoardDAO {
	...
	public boolean boardInsert(BoardVO boarddata) {
		...
		try {
			...
			sql = "insert into BOARD(" + "BOARD_NUM, BOARD_NAME, BOARD_PASS,"
					...
					+ "BOARD_READCOUNT, BOARD_DATE) " + "values((select nvl(max(BOARD_NUM),0)+1 from board),"
					+ "?,?,?,?," // board_content까지
					+ "?,(select nvl(max(BOARD_NUM),0)+1 from board)," // board_re_ref까지
					+ "?,?,?,sysdate)";
	...
	public List<BoardVO> getBoardList(int page, int limit) {
		...
		String board_list_sql = "select * " + "from (select rownum rnum, b.* " + " from (select * from board "
